...
export default function AdminCompanySearch() {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            if (userCredential.user && ALLOWED_ADMIN_EMAILS.includes(userCredential.user.email)) {
            ...
            } else if (error.message === "Access Denied: Not an authorized administrator.") {
    ...
    const performSearch = useCallback(async (term, type, cinOverride = null) => {
        ...
        console.log(`Performing search: term='${currentSearchTerm}', type='${currentSearchType}', cinOverride='${cinOverride}'`); // Log search trigger
        ...
        const CIN_LENGTH = 21;
        if (currentSearchType === 'cin' && !cinOverride && currentSearchTerm.length !== CIN_LENGTH) {
        ...
        if (currentSearchType === 'name' && !cinOverride && currentSearchTerm.length < MIN_NAME_LENGTH) {
        ...
        try {
            ...
            if (apiSuccess) {
                ...
                if (currentSearchType === 'cin' && data.directorRecords && Array.isArray(data.directorRecords)) {
                    foundResults = data.directorRecords;
                    if (cinOverride) setShowingDetailsForCIN(cinOverride); // Mark that we are showing details
                } else if (currentSearchType === 'name' && data.data && Array.isArray(data.data)) {
                    foundResults = data.data;
                } else if (data.data && !Array.isArray(data.data)) { // Handle single object response for CIN?
                    foundResults = [data.data];
            ...
            if (foundResults.length > 0) {
                ...
                } else if (response.ok && !apiSuccess) { // API responded OK but operation failed (e.g., success: false)
    ...
    const debouncedNameSearchRef = useRef(
        debounce((term) => {
            ...
            if (term && term.length >= MIN_NAME_LENGTH) {
                console.log('Term long enough, calling performSearch for name');
                performSearch(term, 'name');
            } else if (term.length === 0) {
                console.log('Name term cleared, clearing results');
                setResults([]); setError(null); setShowingDetailsForCIN(null);
            } else {
                console.log('Name term too short, clearing results');
                setResults([]);
            }
        }, 500) // 500ms delay
    );
    ...
    useEffect(() => {
        ...
        if (searchType === 'name') {
            console.log(`useEffect triggered for name search with term: ${searchTerm}`);
    ...
    const handleDelete = async (recordId, cin) => {
        if (!recordId || !user) { // Check auth
            console.error("Cannot delete: Record ID is missing or user not logged in.");
            setDeleteStatus({ loading: false, error: "Cannot delete record: ID missing or not authorized.", success: null });
            return;
        }

        // Confirmation dialog
        if (!window.confirm(`Are you sure you want to delete the record for CIN: ${cin}? This action cannot be undone.`)) {
            return;
        }
        ...
        try {
            ...
            if (response.data?.success) {
                ...
                setResults(prevResults => prevResults.filter(r => r._id !== recordId));
            } else {
            ...
            const errorMsg = err.response?.data?.message || err.message || "An unexpected error occurred during deletion.";
    ...
    const handleManualSearch = (e) => {
        ...
        if (searchTerm && ((searchType === 'cin' && searchTerm.length === 21) || (searchType === 'name' && searchTerm.length >= 3))) {
             performSearch(searchTerm, searchType);
        } else if (searchType === 'cin' && searchTerm.length !== 21 && searchTerm.length > 0) { // Only show error if something is entered
             setError('Please enter a full 21-character CIN.');
             setResults([]);
        } else if (searchType === 'name' && searchTerm.length < 3 && searchTerm.length > 0) { // Only show error if something is entered
             setError('Please enter at least 3 characters for name search.');
             setResults([]);
        } else if (!searchTerm) { // Clear error if search term is empty
            setError(null);
            setResults([]);
        }
    };
    ...
    const handleUpload = async (event) => {
        ...
        if (!selectedFile || !user) { // Check auth
        ...
        try {
            ...
            if (response.data?.success) {
                ...
                setUploadSuccess(response.data.message || `Upload successful. Inserted ${response.data.insertedCount || 0}, Skipped ${response.data.skippedCount || 0}.`);
                ...
                setUploadError(response.data?.message || "Upload failed. Please check the file or server logs.");
            }
        } catch (err) {
            console.error("Upload Error:", err);
            // Extract message from Axios error response or fallback
            const errorMsg = err.response?.data?.message || err.message || "An unexpected error occurred during upload.";
    ...
    // --- Loading State ---
    if (loadingAuth) {
        return <div className="flex justify-center items-center h-screen"><Loader2 className="h-8 w-8 animate-spin" /> <span className="ml-2">Loading Authentication...</span></div>;
    }

    // --- Auth Error State ---
     if (errorAuth) {
        console.error("Firebase Auth Error:", errorAuth);
        return <p className="text-red-500 text-center mt-10">Error loading authentication state. Please try refreshing the page.</p>;
    }
                    ...
                    <form onSubmit={handleLogin} className="space-y-6">
                        <div>
                            ...
                            <input
                                ...
                                value={loginEmail} onChange={(e) => setLoginEmail(e.target.value)} required disabled={loginLoading}
                            />
                        </div>
                        <div>
                            ...
                            <input
                                ...
                                value={loginPassword} onChange={(e) => setLoginPassword(e.target.value)} required disabled={loginLoading}
                            />
                        </div>
                        {loginError && <p className="text-sm text-red-600">{loginError}</p>}
                        <div>
                            <button className="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-50" type="submit" disabled={loginLoading}>
    ...
    return (
        <div className="container mx-auto p-4 pt-6 md:pt-18 lg:pt-16 pb-10"> {/* Adjusted padding */}
            {/* Header */}
            <div className='mb-6 bg-gradient-to-r from-blue-500 to-indigo-600 rounded-lg shadow-lg mt-2 pt-6 pb-6 relative'> {/* Added relative positioning */}
                <h1 className='text-2xl md:text-3xl font-bold text-center text-white'>ADMIN DASHBOARD</h1>
                {/* Optional: Add a Logout Button */}
                 <button
                    onClick={() => auth.signOut()}
                    className="absolute top-1/2 right-4 cursor-pointer transform -translate-y-1/2 text-white underline hover:text-indigo-200 text-sm"
                    title="Logout"
                 >
                    Logout
                 </button>
            ...
            <div className="p-4 md:p-6 bg-white border border-gray-200 rounded-lg shadow-sm mb-8">
                <h2 className="text-xl md:text-2xl font-semibold mb-4 text-gray-700">Search Companies</h2>
                <form onSubmit={handleManualSearch} className="flex flex-col sm:flex-row gap-3 mb-4 items-stretch sm:items-center">
                    <select
                        value={searchType}
                        onChange={(e) => {
                        ...
                        className="p-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 h-10 sm:w-auto" // Fixed height
                    ...
                    <input
                        ...
                        placeholder={searchType === 'cin' ? 'Enter 21-character CIN' : 'Company Name (min 3 chars)'}
                    ...
                    <button
                        type="submit"
                        disabled={loading || !searchTerm || (searchType === 'cin' && searchTerm.length !== 21) || (searchType === 'name' && searchTerm.length < 3)}
                    ...
                    >
                        {loading && !showingDetailsForCIN ? <Loader2 className="h-5 w-5 animate-spin" /> : 'Search'}
                    </button>
                </form>

                {/* Status Messages Area */}
                 <div className="min-h-[40px] mb-4"> {/* Reserve space for messages */}
                    {error && <p className="text-red-600 text-sm p-2 bg-red-50 rounded border border-red-200">{error}</p>}
                    {deleteStatus.error && <p className="text-red-600 text-sm p-2 bg-red-50 rounded border border-red-200">Delete Error: {deleteStatus.error}</p>}
                    {deleteStatus.success && <p className="text-green-600 text-sm p-2 bg-green-50 rounded border border-green-200">{deleteStatus.success}</p>}
                 </div>


                {/* Search Results */}
                {loading && !results.length && <div className="flex justify-center items-center p-4"><Loader2 className="h-6 w-6 animate-spin text-indigo-600" /></div>}

                {results.length > 0 && (
                    <div className="mt-6">
                        <h3 className="text-lg font-semibold mb-3 text-gray-600">
                            Search Results {showingDetailsForCIN ? `(Details for ${showingDetailsForCIN})` : ''} ({results.length})
                        </h3>
                        <ul className="space-y-3">
                            {results.map((record) => (
                                <li key={record._id || record.CIN || record.cin} className="p-4 border border-gray-200 rounded-lg flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:shadow-md transition-shadow duration-150">
                                    <div className="flex-grow min-w-0">
                                        <p className="font-semibold text-gray-800 truncate" title={record.company || record.CompanyName}>{record.company || record.CompanyName || 'N/A'}</p>
                                        <p className="text-sm text-gray-500">CIN: {record.cin || record.CIN || 'N/A'}</p>
                                        {record.DirectorFirstName && (
                                            <p className="text-sm text-indigo-700 truncate" title={`${record.DirectorFirstName || ''} ${record.DirectorLastName || ''} (DIN: ${record.DirectorDIN || 'N/A'})`}>
                                                Director: {`${record.DirectorFirstName || ''} ${record.DirectorLastName || ''}`.trim() || 'N/A'}
                                                {record.DirectorDIN && ` (DIN: ${record.DirectorDIN.toString()})`}
                                            </p>
                                        )}
                                    </div>
                                    <div className="flex-shrink-0 flex gap-2 mt-2 md:mt-0"> {/* Use flex gap */}
                                        {/* Decide button based on whether it's a full record (_id) or a search result (CIN) */}
                                        {record._id ? (
                                            <button
                                                onClick={() => handleEdit(record._id)}
                                                className="px-3 py-1 text-sm cursor-pointer bg-green-600 text-white rounded-md shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 disabled:opacity-50"
                                            ...
                                            >
                                             ...
                                             record.CIN && !showingDetailsForCIN && // Show "View Records" only on name search results
                                            <button
                                                onClick={() => handleViewDetails(record.CIN)}
                                                className="px-3 py-1 text-sm cursor-pointer bg-blue-600 text-white rounded-md shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50"
                                                disabled={loading}
                                        ...
                                        {/* Add Delete Button - Show only when we have a record ID */}
                                        {record._id && (
                                            <button
                                                onClick={() => handleDelete(record._id, record.cin || record.CIN)}
                                                className="px-3 py-1 text-sm cursor-pointer bg-red-600 text-white rounded-md shadow-sm hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 disabled:opacity-50 flex items-center justify-center"
                                                disabled={loading || deleteStatus.loading} // Disable while loading or deleting
                                                title={`Delete record ${record._id}`}
                                            >
                                                {deleteStatus.loading ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Delete'}
                                            </button>
                                        )}
                                    </div>
                                </li>
                ...
                {/* No Results Message */}
                {!loading && results.length === 0 && !error && searchTerm && (searchType === 'cin' ? searchTerm.length === 21 : searchTerm.length >= 3) && (
                    <p className="text-gray-500 mt-4 text-center">No records found matching your criteria.</p>
                )}
            </div>
            ...
            <div className="p-4 md:p-6 bg-white border border-gray-200 rounded-lg shadow-sm">
                <h2 className="text-xl md:text-2xl font-semibold mb-4 text-gray-700">Upload Company CSV</h2>
                <form onSubmit={handleUpload} className="space-y-4">
                    <div>
                     ...
                     <div className=""> {/* Reserve space for messages */}
                        {uploadError && <p className="text-red-600 text-sm p-2 bg-red-50 rounded border border-red-200">{uploadError}</p>}
                        {uploadSuccess && <p className="text-green-600 text-sm p-2 bg-green-50 rounded border border-green-200">{uploadSuccess}</p>}
                     </div>
                    ...
                    >
                        {uploadLoading ? <Loader2 className="h-5 w-5 animate-spin" /> : 'Upload CSV'}
                    </button>
                </form>
            </div>
        </div>
