...
const StatsSectionJSX = () => (
  <section className="py-12 lg:py-14 xl:py-16 px-4 sm:px-6 md:px-8 gap-6 bg-[#0D2483]">
    {/* Subtitle */}
    <span className="flex items-center justify-center text-base md:text-lg lg:text-xl text-ocean-mist max-w-3xl mx-auto">
...
export default function Home() {
 ...
 const fetchSuggestions = useCallback(
  async (query) => {
    ...
    // DETECT SEARCH TYPE
    // ======================
    const isCinSearch = /^[LU]/i.test(query);      // ❌ DO NOT CHANGE
    const isDinSearch = /^\d{6,}$/.test(query);    // ✅ DIN only numbers
    ...
    /* =====================================================
       👉 DIN SEARCH (ALWAYS DIRECTOR API)
    ===================================================== */
    if (isDinSearch) {
      try {
        const response = await axios.get(DIRECTOR_API_ENDPOINT, {
          params: { din: query }, // ✅ FIX
        });
    ...
    if (searchMode === "company") {
      ...
      try {
        ...
        console.log("Personal API empty");
      }
      ...
      const unique = Object.values(
        finalSuggestions.reduce((acc, item) => {
          if (item.CIN && !acc[item.CIN]) acc[item.CIN] = item;
          return acc;
        }, {})
      );
    ...
    <>
      <main className="flex-1 min-h-screen flex flex-col overflow-x-hidden">
          ...
          <div className="relative mb-16 pt-32 md:pt-36 lg:pt-44 xl:pt-44 px-4 sm:px-6 md:px-16 lg:px-20 xl:px-32">
            <div className="flex flex-col items-center text-white w-full">
              ...
              <h2 className="max-w-4xl text-center font-medium tracking-wide text-gray-200 mt-8 md:mt-14 md:text-lg px-4 sm:px-6 mx-auto">
                ...
                Discover additional information about company financials,
                director details, and more…
              </h2>
            </div>
          </div>
        </section>
        ...
        <section className="py-10 md:py-14 space-y-6">
          ...
          <div className="grid gap-6 sm:gap-8 md:gap-10 lg:gap-12 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:max-w-6xl mx-auto px-4 sm:px-6 md:px-8">
            {[
              {
                icon: LuShieldCheck,
