...
const fetchPaymentHistory = async (userId) => {
    ...
    try {
        ...
        if (response.data && response.data.success) {
            ...
            throw new Error(response.data.message || "Failed to fetch payment history from API.");
        }
    } catch (error) {
        console.error("API Error fetching payment history:", error.response?.data || error.message);
        // Rethrow a user-friendly message or the specific error message
        throw new Error(error.response?.data?.message || error.message || "Could not connect to fetch payment history.");
    }
};
// --- End Fetch Function ---
...
const DashboardPage = () => {
        ...
        if (user) {
            ...
            fetchPaymentHistory(user.uid)
                .then(data => {
                    ...
                    const sortedData = [...data].sort((a, b) => new Date(b.paymentDate || b.createdAt) - new Date(a.paymentDate || a.createdAt)); // Use paymentDate or createdAt
    ...
    if (authLoading) {
        return (
            <div
                ...
                role="status"
            >
                 <Loader2 className="h-8 w-8 animate-spin text-primary mr-3" /> {/* Use primary color */}
                <p className="text-lg text-muted-foreground">Loading your dashboard...</p> {/* Use theme colors */}
            </div>
        );
    }
    ...
    // --- Render Dashboard ---
    return (
        // Use theme background, add padding, ensure min height
        <div className="container-fluid px-4 sm:px-6 lg:px-8 py-8 min-h-[calc(100vh-80px)] md:mt-14">
            ...
            <div className="bg-card text-card-foreground shadow-lg rounded-xl p-4 md:p-8 max-w-7xl mx-auto">
                ...
                <div className="mt-6 border-t border-border pt-6"> 
                    <h2 className="text-2xl font-semibold text-foreground mb-5">Payment History (Contact Unlocks)</h2>
                    ...
                    {historyError && !historyLoading && (
                        <div role="alert" className="text-center py-6 text-destructive bg-destructive/10 p-4 rounded-lg border border-destructive/30">
                    ...
                    {!historyLoading && !historyError && (
                        paymentHistory.length > 0 ? (
                            <div className="overflow-x-auto shadow-sm border border-border rounded-lg">
                                <table className="min-w-full divide-y divide-border">
                                    <thead className="bg-muted/50"> 
                                        <tr>
                                            <th scope="col" className="px-5 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">Date</th>
                                            <th scope="col" className="px-5 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">Amount</th>
                                            <th scope="col" className="px-5 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">Director Unlocked</th>
                                            <th scope="col" className="px-5 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">Payment ID</th>
                                            <th scope="col" className="px-5 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">Status</th>
                                        </tr>
                                    </thead>
                                            ...
                                            <tr key={payment._id || payment.id} className="hover:bg-muted/30 transition-colors">
                                                ...
                                                <td className="px-5 py-4 whitespace-nowrap text-sm text-muted-foreground">
                                                    <div className="flex items-center">
                                                        ...
                                                        <div>
                                                            <div className="font-medium text-foreground">{payment.directorName || 'N/A'}</div>
                                                            <div className="text-xs text-gray-500">DIN: {payment.dinUnlocked || 'N/A'}</div>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td className="px-5 py-4 whitespace-nowrap text-sm text-muted-foreground font-mono">
                                                    <div className="flex items-center" title={payment.razorpay_payment_id}>
                                                        <Hash className="w-3 h-3 mr-1.5 text-gray-400 flex-shrink-0" />
                                                        {/* Displaying only part of the ID for brevity */}
                                                        {payment.razorpay_payment_id?.substring(4) || 'N/A'}
                                                    </div>
                                                </td>
                                                <td className="px-5 py-4 whitespace-nowrap text-sm">
                                                    {/* Using theme colors for status badges */}
                                                    {payment.status === 'captured' || payment.status === 'paid' ? (
                                                        <span className="px-2.5 py-0.5 inline-flex items-center text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300">
                                                            <CheckCircle className="w-3 h-3 mr-1" /> Successful
                                                        </span>
                                                    ) : (
                                                        <span className="px-2.5 py-0.5 inline-flex items-center text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300">
                                                            <AlertCircle className="w-3 h-3 mr-1" /> {payment.status ? payment.status.charAt(0).toUpperCase() + payment.status.slice(1) : 'Failed'}
                                                        </span>
                                                    )}
                                                </td>
                                            </tr>
                            ...
                            <div className="text-center py-10 border border-dashed border-border rounded-lg bg-background">
                                <p className="text-muted-foreground">You have no payment history for contact unlocks yet.</p>
                            </div>
                        )
                    )}
                </div>
